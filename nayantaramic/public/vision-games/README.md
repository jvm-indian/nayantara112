# ðŸŒŸ Neon Aura AR â€” Hand & Face Tracking Experience

> A browser-based Augmented Reality experience powered by **MediaPipe AI** â€” no installs, no plugins, just your webcam and imagination.
...
- Let you **play gesture-controlled games** â€” all in the browser

Everything runs locally using vanilla HTML, CSS, and JavaScript â€” zero backend required.

---

## âœ¨ Features

### ðŸ– Hand Tracking AR (`index.html`)
| Feature | Description |
|---|---|
...
| **5 Color Themes** | Rainbow, Cyberpunk, Lava, Ocean, Galaxy â€” switchable live |
| **Web Audio Engine** | Ambient hum that changes pitch/volume based on hand distance |

### ðŸ‘ Face Detection (`index.html` â€” toggleable)
| Feature | Description |
|---|---|
...
| **Toggle On/Off** | Enable/disable independently â€” saves CPU when not needed |

### ðŸŽ® Hand Gesture Games (`games.html`)
#### ðŸŽï¸ Neon Car Racer
...
- Raise **â˜ï¸ index finger** for a speed boost
- Live **leaderboard** and animated **speedometer**

#### ðŸƒ City Marathon Run
- **5 runners** race together â€” you + 4 AI NPCs with unique looks
...
- **â˜ï¸ Index Up** = Jump | **âœŠ Fist** = Duck
...
| Gesture | Effect |
|---|---|
| âœ‹ **Move hand fast** | Matrix rain accelerates |
| ðŸ‘Œ **Pinch** (thumb + index) | Shockwave + Zap sound |
| ðŸ– **Open Hand** | Full fingertip sparks |
| âœŠ **Fist** | Detected in HUD |
| ðŸ¤² **Both hands close** | Electrical arcs between fingertips |
| ðŸ™Œ **All 10 fingers** | Rotating mandala effect |

### Car Racer (`games.html`)
| Gesture | Effect |
|---|---|
| ðŸ– **Tilt hand left** | Steer car left |
| ðŸ– **Tilt hand right** | Steer car right |
| â˜ï¸ **Index finger up** | Activate BOOST |

### City Marathon Run (`games.html`)
| Gesture | Effect |
|---|---|
| â˜ï¸ **Index finger up** | Jump over obstacles |
| âœŠ **Fist** | Duck under laser beams |
...
connect/
â”‚
â”œâ”€â”€ index.html        # Main AR experience (hand + face tracking, themes, audio)
â”œâ”€â”€ games.html        # Hand gesture games hub (car racer + endless runner)
...
### Option 1 â€” Direct in Browser
Just open `index.html` in any modern browser (Chrome recommended).

> âš ï¸ **Note:** Your browser will ask for **camera permission** â€” this is required for hand/face tracking. No video is stored or transmitted anywhere.
...
# VS Code â€” install "Live Server" extension and click "Go Live"
```
...
| Browser | Status |
|---|---|
| âœ… Google Chrome | Fully supported (recommended) |
| âœ… Microsoft Edge | Fully supported |
| âš ï¸ Firefox | Partial (WebGL may differ) |
| âŒ Safari | Limited (WebRTC constraints) |
...
- `bgCanvas` â€” renders the matrix rain background with trailing fade effect
- `mainCanvas` â€” renders hand/face overlays using `globalCompositeOperation: 'screen'` for neon bloom

### Lazy Face Mesh Loading
Face detection is **off by default**. The `FaceMesh` WASM model is only downloaded and initialized the first time you click **"ðŸ‘ Face: ON"** â€” keeping startup fast and saving CPU for users who don't need it.
...
```
Camera frame â†’ Hands model  â†’ currentHands[] â†’ render loop
            â†’ FaceMesh model (if ON) â†’ currentFaces[] â†’ render loop
```
...
- **[MediaPipe](https://mediapipe.dev/)** by Google â€” AI hand & face tracking
- **[Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)** â€” Browser-native sound synthesis
- **[Google Fonts â€” Orbitron & Inter](https://fonts.google.com/)** â€” Typography

---

*Built with â¤ï¸ by **Nayanthara Team** Â· B.Tech CS (AIML) Â· Nayanthara Team*

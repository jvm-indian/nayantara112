...
export default function HiddenHerb({ childId, currentLevel, onLevelComplete }: HiddenHerbProps) {
  const levelConfig = HIDDEN_HERB_LEVELS[currentLevel - 1];
  ...
  useEffect(() => {
    ...
    items.push({
      ...
      vx: levelConfig.features.includes('Moves Slowly') ? (Math.random() - 0.5) * 0.3 : 0,
      vy: levelConfig.features.includes('Moves Slowly') ? (Math.random() - 0.5) * 0.3 : 0,
    });
  ...
  const handleGameEnd = (status: 'WON' | 'FAILED') => {
    ...
    const metrics: GameLog = {
      ...
      ai_insight: generateGitaWisdom('Child', 'HIDDEN_HERB', status === 'WON', currentLevel),
      recommended_action: '',
    };
      ...
      <motion.div className="w-full max-w-4xl mb-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="font-body text-ayur-slate/60">Level {currentLevel} of 11 - Find the Glowing Leaf</p>
          </div>
